...
class queueCustomer
{
	...
	public:
		...
		void push(const informCustomer & data, bool priorityOrder)   // data.Num - temp->data.Num = -1;
		{
			if (priorityOrder)
			{
				if (this->head== NULL)
...
class AVL
{
	...
	public:
		...
		void clearLRN(Node*& root)
		{
			if (root)
			{
				this->clearLRN(root->left);
				this->clearLRN(root->right);
				delete root;
				root= NULL;
			}
		}
...
class huffTree
{
	...
	public:
		...
		void heapify(int index)
		{
			...
			if (min != index)
			{
				this->swap(this->Array[min], this->Array[index]);
				this->heapify(min);
			}
		}
		void pop()
		...
		void push(Node*& temp)
		{
			...
			else
			{
				...
				while (i > 0)
				{
					...
					{
						this->swap(this->Array[i], this->Array[parent]);
						i= parent;
					}
					else
					{
						if (this->Array[parent]->freq== this->Array[i]->freq)
						{
							...
							else
							{
								if (this->Array[parent]->character > this->Array[i]->character)
								{
									this->swap(this->Array[parent], this->Array[i]);
									i= parent;
								}
								else
									break;
...
void Statics(string & informString, int Array [], int size)
{
	for (int i= 0; i < size; i++)
		Array[i]= 0;
	for (int i= 0; i < int(informString.size()); i++)
	{
					...
					if (instructionString == "CLE")
					{
						...
						else
						{
							...
							else
							{
								...
								if (informString[0]== '-')
								{
									for (int i= 1; i < int(informString.size()); i++)
									{
